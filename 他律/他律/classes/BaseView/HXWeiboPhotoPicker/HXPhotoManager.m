...
- (void)FetchAllAlbum:(void(^)(NSArray *albums))albums IsShowSelectTag:(BOOL)isShow
{
    ...
    [smartAlbums enumerateObjectsWithOptions:NSEnumerationConcurrent usingBlock:^(PHAssetCollection *collection, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        // 过滤掉空相册
        if (result.count > 0 && ![[HXPhotoTools transFormPhotoTitle:collection.localizedTitle] isEqualToString:@"Recently Deleted"]) {
...
//            albumModel.asset = result.lastObject;
            ...
            if ([[HXPhotoTools transFormPhotoTitle:collection.localizedTitle] isEqualToString:@"Camera Roll"] || [[HXPhotoTools transFormPhotoTitle:collection.localizedTitle] isEqualToString:@"All Photos"]) {
