...
@interface TLNewestLvyouQuanCell ()
...
@implementation TLNewestLvyouQuanCell

+ (instancetype)cellWithTableView:(UITableView *)tableView withIdentifer:(NSString *)identifier
{
    TLNewestLvyouQuanCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil)
    {
        cell = [[TLNewestLvyouQuanCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
        [cell setupUI];
    }
    return cell;
}
...
-(void)setModel:(TLLvyouQuanModel *)model {
    _model = model;
    if (![model.authorName isEqualToString:@"他律官方号"]) {
        self.authorLbl.text = model.authorName;
        self.authorLbl.textColor = hexStringToColor(@"333333");
        self.iconView.hidden = YES;
    }else {
        self.authorLbl.text = @"他律官方号";
        self.authorLbl.textColor = hexStringToColor(@"26c239");
...
-(void)layoutSubviews {
    ...
    self.bgView.frame = CGRectMake(0, 10 * layoutBy6(), J_SCREEN_WIDTH, self.model.cellHeight - 10 * layoutBy6());
    ...
    self.iconView.frame = CGRectMake(self.authorLbl.right + 5.5 * layoutBy6(), self.authorLbl.top, 13.5 * layoutBy6(), 13.5 * layoutBy6());
...
-(UIImageView *)iconView {
    if (!_iconView) {
        ...
        _iconView.image = [UIImage imageNamed:@"icon_guanfang"];
    }
    return _iconView;
}
