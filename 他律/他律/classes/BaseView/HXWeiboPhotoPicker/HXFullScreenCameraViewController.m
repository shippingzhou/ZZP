...
//        navItem.rightBarButtonItems = @[rightOne_];
    ...
    if (self.type == HXCameraTypePhotoAndVideo) {
        self.titleStr = @"Click to take a photo, long press the video";
        ...
        self.titleStr = @"Click to take photos";
        if ([self.session canAddOutput:self.imageOutput]) {
            [self.session addOutput:self.imageOutput];
        }
    }else if (self.type == HXCameraTypeVideo) {
        self.titleStr = @"Long press video";
        if ([self.session canAddOutput:self.videoOutPut]) {
            [self.session addOutput:self.videoOutPut];
        }
    }
    ...
    backBtn.frame = CGRectMake(15, 20, backBtn.currentImage.size.width + 20, backBtn.currentImage.size.height);
    ...
    self.nextBtn.frame = CGRectMake(WIDTH - 10 - 60, HEIGHT - 20 - 25, 80, 25);
...
- (void)playViewTapGrEvent:(UITapGestureRecognizer *)tap {
    ...
    if (!conntion) {
        [self.view showImageHUDText:@"Photo failed"];
        return;
    }
...
- (void)hideClick {
    ...
    [self.backBtn setTitle:@"Retake" forState:UIControlStateNormal];
...
- (void)playViewLongPgrEvent:(UILongPressGestureRecognizer *)longPgr {
    ...
    if (longPgr.state == UIGestureRecognizerStateCancelled ||
        ...
        if (self.videoTime < 3) {
            ...
            [self.view showImageHUDText:@"Recording time cannot be less than 3 seconds"];
            self.timeLb.text = self.titleStr;
        }else {
            [self hideClick];
        }
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didStartRecordingToOutputFileAtURL:(NSURL *)fileURL fromConnections:(NSArray *)connections { 
    // 开始录制
    self.videoTime = 0;
    self.timeLb.text = [NSString stringWithFormat:@"0 seconds"];
...
- (void)recordingClick:(NSTimer *)timer {
    self.videoTime++;
    self.timeLb.text = [NSString stringWithFormat:@"%ldseconds",self.videoTime];
