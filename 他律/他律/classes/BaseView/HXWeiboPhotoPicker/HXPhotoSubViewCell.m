...
- (void)didDeleteClick {
    if (self.model.networkPhotoUrl.length > 0) {
        if (self.showDeleteNetworkPhotoAlert) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"prompt" message:@"Whether to delete this photo" delegate:self cancelButtonTitle:@"cancel" otherButtonTitles:@"determine", nil];
            [alert show];
            return;
        }
    }
