...
- (void)setupUI {
    ...
    [rightBtn setBackgroundColor:hexStringToColor(@"ffffff")];
...
- (void)clipImage {
    ...
    UIImage *image = [self imageFromView:self.bgView atFrame:CGRectMake(0, (height / 2 - diameter / 2), diameter, diameter)];
...
- (UIImage *)imageFromView: (UIView *) theView   atFrame:(CGRect)r {
   ...
   // CGSize size = CGSizeMake(r.size.width *scale, r.size.height*scale);
    CGSize size2 = CGSizeMake(theView.frame.size.width *scale, theView.frame.size.height*scale);
    UIGraphicsBeginImageContextWithOptions(size2, NO, [UIScreen mainScreen].scale);
    CGContextRef context = UIGraphicsGetCurrentContext();
    CGContextSaveGState(context);
    UIRectClip(CGRectMake(r.origin.x, r.origin.y, r.size.width * 2, r.size.height * 2));
