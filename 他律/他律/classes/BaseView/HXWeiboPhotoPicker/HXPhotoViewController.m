...
@property (nonatomic, assign) BOOL isShoot;//是否是拍摄
@end
...
- (UILabel *)authorizationLb
{
    if (!_authorizationLb) {
        ...
        _authorizationLb.text = @"Unable to access photo\nPlease click here to go to the settings to allow access to photos.";
...
- (void)goCameraVC {
    if (self.manager.goCamera) {
        ...
        if(![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
            [self.view showImageHUDText:@"This device does not support cameras!"];
            return;
        }
        AVAuthorizationStatus authStatus = [AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo];
        if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Unable to use camera" message:@"Please allow access to the camera in Settings - Privacy - Camera" delegate:self cancelButtonTitle:@"cancel" otherButtonTitles:@"setting", nil];
            [alert show];
            return;
        }
...
- (void)setup {
    ...
    if (!self.manager.singleSelected) {
        ...
        if (self.manager.selectedList.count > 0) {
            ...
            [self.rightBtn setTitle:[NSString stringWithFormat:@"Next step(%ld)",self.manager.selectedList.count] forState:UIControlStateNormal];
            ...
            [self.rightBtn setTitle:@"Next step" forState:UIControlStateNormal];
    ...
    [titleBtn setTitle:@"Camera roll" forState:UIControlStateNormal];
    ...
    if (!self.manager.singleSelected) {
        ...
        if (self.manager.selectedPhotos.count > 0 && self.manager.isOriginal) {
            ...
            [bottomView.originalBtn setTitle:[NSString stringWithFormat:@"Original image(%@)",self.manager.photosTotalBtyes] forState:UIControlStateNormal];
...
- (void)cellDidSelectedBtnClick:(HXPhotoViewCell *)cell Model:(HXPhotoModel *)model {
    if (!cell.selectBtn.selected) { // 弹簧果冻动画效果
        if (self.manager.selectedList.count == self.manager.maxNum) {
            [self.view showImageHUDText:[NSString stringWithFormat:@"You can only select up to %ld",self.manager.maxNum]];
            // 已经达到最大选择数
            return;
        }
        if (self.manager.type == HXPhotoManagerSelectedTypePhotoAndVideo) {
            if ((model.type == HXPhotoModelMediaTypePhoto || model.type == HXPhotoModelMediaTypePhotoGif) || (model.type == HXPhotoModelMediaTypeCameraPhoto || model.type == HXPhotoModelMediaTypeLivePhoto)) {
                if (self.manager.videoMaxNum > 0) {
                    if (!self.manager.selectTogether) { // 是否支持图片视频同时选择
                        if (self.manager.selectedVideos.count > 0 ) {
                            [self.view showImageHUDText:@"Pictures cannot be selected at the same time as the video"];
                            // 已经选择了视频,不能再选图片
                            return;
                        }
                    }
                ...
                if (self.manager.selectedPhotos.count == self.manager.photoMaxNum) {
                    [self.view showImageHUDText:[NSString stringWithFormat:@"You can only select up to %ld images",self.manager.photoMaxNum]];
                    // 已经达到图片最大选择数
                    return;
                }
            }else if (model.type == HXPhotoModelMediaTypeVideo || model.type == HXPhotoModelMediaTypeCameraVideo) {
                if (self.manager.photoMaxNum > 0) {
                    if (!self.manager.selectTogether) { // 是否支持图片视频同时选择
                        if (self.manager.selectedPhotos.count > 0 ) {
                            // 已经选择了图片,不能再选视频
                            [self.view showImageHUDText:@"Video cannot be selected at the same time as the picture"];
                            return;
                        }
                    }
                }
                if (self.manager.selectedVideos.count == self.manager.videoMaxNum) {
                    [self.view showImageHUDText:[NSString stringWithFormat:@"You can only select up to %ld videos",self.manager.videoMaxNum]];
                    // 已经达到视频最大选择数
                    return;
                }
            }
        ...
        if (model.type == HXPhotoModelMediaTypeVideo) {
            if (model.asset.duration < 3) {
                [self.view showImageHUDText:@"Video is less than 3 seconds, not supported yet"];
                return;
            }
        }
...
- (void)changeOriginalState:(BOOL)selected IsChange:(BOOL)isChange
{
    if (selected) { // 选中时
        if (isChange) { // 改变成初始状态
            ...
            [self.bottomView.originalBtn setTitle:@"Original image" forState:UIControlStateNormal];
        }
        // 记录原图按钮的frame
        ...
        [HXPhotoTools FetchPhotosBytes:self.manager.selectedPhotos completion:^(NSString *totalBytes) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                [weakSelf.bottomView.originalBtn setTitle:[NSString stringWithFormat:@"Original image(%@)",totalBytes] forState:UIControlStateNormal];
...
- (UIActivityIndicatorView *)indica
{
    if (!_indica) {
        ...
        CGFloat indicaX = self.bottomView.originalBtn.titleLabel.frame.origin.x + [HXPhotoTools getTextWidth:@"Original image" withHeight:self.bottomView.originalBtn.frame.size.height / 2 fontSize:14] + 5;
...
- (UIButton *)rightBtn
{
    if (!_rightBtn) {
        ...
        [_rightBtn setTitle:@"Next step" forState:UIControlStateNormal];
...
- (void)setup {
    ...
    [originalBtn setTitle:@"Original image" forState:UIControlStateNormal];
