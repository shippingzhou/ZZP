...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.networkPhotoUrl.length > 0) {
        if (!model.downloadComplete) {
            [[self viewController:self].view showImageHUDText:@"Photo is downloading"];
            return;
...
- (void)goPhotoViewController {
    if (self.manager.outerCamera) {
        ...
        UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"cancel" destructiveButtonTitle:nil otherButtonTitles:@"camera",@"Album", nil];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 0) {
        if(![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
            [[self viewController:self].view showImageHUDText:@"This device does not support cameras!"];
            return;
        }
        ...
        if (self.manager.type == HXPhotoManagerSelectedTypePhotoAndVideo) {
            if (self.videos.count >= self.manager.videoMaxNum && self.photos.count < self.manager.photoMaxNum + self.networkPhotos.count) {
                ...
                [[self viewController:self].view showImageHUDText:@"The maximum number has been reached!"];
                return;
            }else {
            ...
            if (self.photos.count >= self.manager.photoMaxNum + self.networkPhotos.count) {
                [[self viewController:self].view showImageHUDText:@"The photo has reached the maximum number"];
                return;
            }
            type = HXCameraTypePhoto;
        }else if (self.manager.type == HXPhotoManagerSelectedTypeVideo) {
            if (self.videos.count >= self.manager.videoMaxNum) {
                [[self viewController:self].view showImageHUDText:@"The maximum number of videos has been reached!"];
                return;
            }
            type = HXCameraTypeVideo;
        }
